...
- **Agentic Workflow**: 使用 LangGraph 驱动的多步分析流程（提取 -> 分析 -> 总结）。
- **自动化爬虫**: 自动监测英雄联盟官网最新的版本更新公告。
- **智能缓存机制**: 
    - 系统会自动将分析结果持久化到 `data/cache/{version}.json`。
    - 对于同一版本的请求，系统仅在第一次时执行 AI 分析，后续请求将秒级命中缓存返回。
    - 支持手动清理 `data/cache` 目录以强制重新分析。
